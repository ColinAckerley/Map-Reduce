...
int main(int argc, char **argv){
	...
	readInput();
	/**FOR DEBUGGING
	printf("App: %d, Impl: %d, Maps: %d, Reduces: %d, inputCount %dn", app, impl, maps, reduces,inputCount);
	fflush(stdout);
	struct inputList *inNode = inputHead;
	while(inNode != NULL){
		if(inNode->data != NULL){
...
void mapSetup(){
	...
	if(impl == 1){
		for(x=0; x<maps; x++){
			pthread_join(threadIDs[x],NULL);
			printf("Child thread %d joinedn", (int)threadIDs[x]);
		}
	} else {
		while(wait(NULL) > 0);
	}
